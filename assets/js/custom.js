			...
			$(function(){
				...
				zoomOnScrollSpeed:1, //default value is 3
				 zoomStep:1.1, //default value is 1.6
					...
					// ]
					markers: [
						{ latLng: [-36.848461, 174.763336], name: 'Auckland, New Zealand' },
						{ latLng: [13.0827, 80.2707], name: 'Chennai, Tamil Nadu' },
						{ latLng: [15.2993, 74.1240], name: 'Goa' },
						{ latLng: [26.7606, 83.3732], name: 'Gorakhpur, Uttar Pradesh' },
						{ latLng: [17.3850, 78.4867], name: 'Hyderabad, Telangana' },
						{ latLng: [26.4499, 80.3319], name: 'Kanpur, Uttar Pradesh' },
						{ latLng: [19.0760, 72.8777], name: 'Mumbai, Maharashtra' },
						{ latLng: [21.1458, 79.0882], name: 'Nagpur, Maharashtra' },
						{ latLng: [28.6139, 77.2090], name: 'New Delhi, Delhi' },
						{ latLng: [18.5204, 73.8567], name: 'Pune, Maharashtra' },
						{ latLng: [31.1471, 75.3412], name: 'Punjab' },
						{ latLng: [26.9124, 75.7873], name: 'Rajasthan' },
	...
	if($('.submitForm').length > 0){
			...
			target.find('input , textarea , select').each(function(){
				...
				if($(this).val().trim() != ''){
					...
					if(typeof valid != 'undefined'){
						if(valid == "email" && !email.test($(this).val().trim())){
							$(this).addClass('error');
							$(this).focus();
							check = 1;
							targetResp.html('<p style="color:red;">Email should be valid.</p>');
							return false;
						}else{
							$(this).removeClass('error');
						}
		...
		$('.submitForm').on('click', function(e) {
			...
			if(check == 0){
				...
				// 👇 Create normal form object (NOT FormData)
				var formData = {
					first_name: targetForm.find("input[name='first_name']").val(),
					last_name: targetForm.find("input[name='last_name']").val(),
					email: targetForm.find("input[name='email']").val(),
					subject: targetForm.find("input[name='subject']").val(),
					message: targetForm.find("textarea[name='message']").val()
				};

				$.ajax({
					method: "POST",
					url: "https://script.google.com/macros/s/AKfycbw0dggUJ2FPpriP6mQwMH-FzomDJx65-WGyUZtXPi-owkbAA1Wb5LWtk-fR3wZHDuY/exec",
					data: formData,
					...
					error: function(){
						errroTarget.html('<p style="color:red;">Something went wrong. Please try again.</p>');
					}
				});
			}
		});
